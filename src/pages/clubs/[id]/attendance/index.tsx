...
function AttendancePage({ user, isLoggedIn }: ClubDetailPageProps) {
  ...
  const handleEditSubmit = async (values: WorkoutEditValues) => {
    ...
    const response = await fetch(`/api/workouts/${editTarget.id}`, {
      method: 'PATCH',
  ...
  const handleDeleteConfirm = async () => {
    ...
    const response = await fetch(`/api/workouts/${deleteTarget.id}`, {
      method: 'DELETE',
    });
    const result = await response.json();
    ...
    <>
      ...
      {deleteTarget && (
        <WorkoutDeleteSheet
          ...
          participantCount={deleteTarget.WorkoutParticipant?.length ?? 0}
