...
import { bulkUnconfirmSchema } from '@/schemas/membership-fee.schema';
...
export default withAuth(async function handler(
  ...
  try {
    const parseResult = bulkUnconfirmSchema.safeParse(req.body);
      ...
      try {
        await prisma.$transaction(async (tx) => {
          await tx.membershipPayment.deleteMany({
            where: { paymentRecordId: { in: targetIds } },
          });
    ...
    return res.status(200).json({
      ...
      status: 200,
      message: `${results.success.length}건 확정 취소, ${results.failed.length}건 실패`,
    });
  } catch (error) {
    console.error('Error in bulk unconfirm:', error);
    return res.status(500).json({
      error: '일괄 확정 취소 처리 중 오류가 발생했습니다',
      status: 500,
    });
  }
});
